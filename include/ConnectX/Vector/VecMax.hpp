#ifndef _CONNECT_X_VECTOR_VECMAX_HPP_
#define _CONNECT_X_VECTOR_VECMAX_HPP_
...
namespace ConnectX::VectorExpression {
  ...
  public:
    ...
    auto operator[](std::size_t index) const {
      return std::max(m_u[index], m_v[index]);
    }
...
namespace ConnectX {
  template <typename U, typename V>
  auto Max(ConnectX::VectorExpression::VecEx<U> const& lhs, ConnectX::VectorExpression::VecEx<V> const& rhs) {
    return ConnectX::VectorExpression::VecMax<U,V>(lhs, rhs);
  }
}

#endif // #ifndef _CONNECT_X_VECTOR_VECMAX_HPP_