#ifndef _CONNECT_X_VECTOR_VECGREATEREQ_HPP_
#define _CONNECT_X_VECTOR_VECGREATEREQ_HPP_
...
namespace ConnectX::VectorExpression {
  ...
  class VecGreaterEq : public VecEx<VecGreaterEq<U,V>> {
  public:
    using TU = std::conditional_t<U::Leaf, U const&, U>;
    using TV = std::conditional_t<V::Leaf, V const&, V>;

    VecGreaterEq(VecEx<U> const& u, VecEx<V> const& v)
...
auto operator>=(ConnectX::VectorExpression::VecEx<U> const& lhs, ConnectX::VectorExpression::VecEx<V> const& rhs) {
  return ConnectX::VectorExpression::VecGreaterEq<U,V>(lhs, rhs);
}
